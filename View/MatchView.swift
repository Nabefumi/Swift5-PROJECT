...
class MatchView: UIView {
    ...
    private let sendMessageButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(didTopSendSwiping), for: .touchUpInside)
        return button
    }()
    
    private let keepSwioingButton: UIButton = {
    ...
    func configureUI() {
        ...
        keepSwioingButton.anchor(top: sendMessageButton.bottomAnchor, left: leftAnchor, right: rightAnchor, paddingTop: 32, paddingLeft: 48, paddingRight: 48)
        keepSwioingButton.heightAnchor.constraint(equalToConstant: 60).isActive = true
        
        descriptionLabel.anchor(left: leftAnchor, bottom: currentUserImageView.topAnchor, right: rightAnchor, paddingBottom: 32, height: 50)
    ...
    func configureAnimations() {
        ...
        currentUserImageView.transform = CGAffineTransform(rotationAngle:
                                                            -angle).concatenating(CGAffineTransform(translationX: 200, y: 0))
        matchedUserImageView.transform = CGAffineTransform(rotationAngle:
                                                            angle).concatenating(CGAffineTransform(translationX: -200, y: 0))
        
        self.sendMessageButton.transform = CGAffineTransform(translationX: -500, y: 0)
        self.keepSwioingButton.transform = CGAffineTransform(translationX: 500, y: 0)
        ...
        UIView.animate(withDuration: 0.75, delay: 0.6 * 1.3, usingSpringWithDamping: 0.5,
            ...
            self.keepSwioingButton.transform = .identity
        }, completion: nil)
    }
